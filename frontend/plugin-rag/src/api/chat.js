...
const API_BASE = typeof window !== 'undefined'
    ? (window.__RAG_API_BASE__ || 'http://localhost:8000/api')
 ...
 */
export async function generalChat(query, topK = 5, sessionId = null, userId = null) {
...
export async function getThreadMessages(threadId, userId) {
    const response = await fetch(`${API_BASE}/history/${threadId}/messages?user_id=${userId}`, {
        method: 'GET',
    });
...
export async function deleteThread(threadId, userId) {
    const response = await fetch(`${API_BASE}/history/${threadId}?user_id=${userId}`, {
        method: 'DELETE',
    });
